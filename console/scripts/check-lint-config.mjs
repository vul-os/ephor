...
//
// Warnings are deliberately NOT gated here. console/ currently carries a
// handful (~4) of pre-existing warnings; failing on warning count would turn
// this gate red for reasons unrelated to whatever a given change actually
// did. Errors are the correctness bar (a rule configured as `error` in
// eslint.config.js means "this must not ship"); warnings are surfaced in the
// PASS message for visibility only, not enforced.
// ============================================================================
