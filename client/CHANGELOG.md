# Changelog — the relay client SDK and the Go reverse-tunnel relay

Versions 0.1.0–0.3.0 here predate the rename of this project from Ephor to Pier. They document the
JS SDK (`client/`) and the Go relay (`tunnel/`, `cmd/`) — a package's version history, which had
been sitting in the repo-root CHANGELOG describing releases the *repo* never made. Moved here
unedited: it records what shipped, and rewriting it would falsify that.

The repo-root `CHANGELOG.md` now tracks **Pier the project** and starts at the rename.
