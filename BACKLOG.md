...
Autonomous program: complete **Pier** (broker reference implementation of the KOTVA
...
- [ ] W0.1 Rewrite pier `README.md` → Pier = broker reference impl (the kinds, no-token billing, self-host). Clean OSS.
...
## Phase 1 — pier-economics (the billing model) [CORE, mostly unblocked]
...
- [ ] E1.7 Tests + rustdoc for pier-economics.
...
- [ ] K2.4 gateway (mail adapter) — complete per §7 (already folded); wire to pier-economics.
...
- [ ] C4.1 pier-conformance — assert each kind meets COORD-1..8 and its visibility declaration matches reality.
- [x] C4.2 CI (GitHub Actions) — `rust` job in `.github/workflows/ci.yml`: `cargo build`/`test`/`clippy -D warnings`/`fmt --all --check`, all `--locked`, over the 14 workspace crates (the `pier-conformance` COORD-1..8 harness runs inside that `cargo test`). Guarded by a crate-count assertion (must resolve exactly 14) and a test-count floor (≥556 passed), both proven to fail on planted breakage. **Not covered:** `crates/pier-billing-patala` (excluded from the workspace — needs the sibling `patala` repo); the job prints that exclusion on every run.

## Phase 5 — envoir node-only
- [ ] N5.1 Remove the gateway crate from envoir (it now lives in pier).
...
## Done criteria
Pier: all kinds present (real or contract-stubbed), pier-economics billing model complete,
