//! The structured operator policy carried in a [`pier_economics::Descriptor::policy`]'s opaque
//! `Cbor` payload (CONTRACT §2.1: "opaque operator policy — region, capabilities, contact").
//!
//! `pier_economics` deliberately leaves this payload's shape to the caller (it only carries and
...
impl OperatorPolicy {
    ...
    /// Encode as the [`Cbor`] payload a [`pier_economics::Descriptor::policy`] carries.
