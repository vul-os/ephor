//! Binary config for `pier-admin` (`src/main.rs`), read from the environment. A library
...
use pier_economics::{AssuranceLevel, ContentVisibility, CoordinatorKind, VisibilityClass};

/// `PIER_ADMIN_BIND` — defaults to loopback (operator-local; see `auth.rs`/`lib.rs` module
...
pub struct AdminBinConfig {
    ...
    /// `None` = no `PIER_ADMIN_TOKEN` configured — the server still starts, but
    /// [`crate::auth::AdminAuth::disabled`] refuses every request (fail-closed, SEC-1).
    pub admin_token: Option<String>,
    pub kind: CoordinatorKind,
    pub visibility: ContentVisibility,
    /// A 32-byte Ed25519 seed, if `PIER_ADMIN_KEY_SEED_HEX`/`PIER_ADMIN_KEY_FILE` was
...
impl AdminBinConfig {
    pub fn from_env() -> Result<Self, String> {
        let bind_addr = match std::env::var("PIER_ADMIN_BIND") {
            Ok(v) => v
                .parse()
                .map_err(|e| format!("PIER_ADMIN_BIND {v:?} is not a socket address: {e}"))?,
            Err(_) => DEFAULT_BIND.parse().expect("DEFAULT_BIND is valid"),
        };

        let admin_token = std::env::var("PIER_ADMIN_TOKEN").ok();

        let kind = match std::env::var("PIER_ADMIN_KIND") {
            Ok(v) => CoordinatorKind::from_wire_str(&v)
                .ok_or_else(|| format!("PIER_ADMIN_KIND {v:?} is not a known coordinator kind"))?,
            Err(_) => CoordinatorKind::Relay,
        };

        let visibility = match (
            std::env::var("PIER_ADMIN_VISIBILITY_CLASS").ok(),
            std::env::var("PIER_ADMIN_VISIBILITY_LEVEL").ok(),
        ) {
            ...
            _ => {
                return Err(
                    "PIER_ADMIN_VISIBILITY_CLASS and PIER_ADMIN_VISIBILITY_LEVEL must both \
        ...
        let key_seed = match std::env::var("PIER_ADMIN_KEY_SEED_HEX") {
            Ok(v) => Some(parse_seed_hex(&v)?),
            Err(_) => match std::env::var("PIER_ADMIN_KEY_FILE") {
                Ok(path) => {
                    let contents = std::fs::read_to_string(&path)
                        .map_err(|e| format!("reading PIER_ADMIN_KEY_FILE {path:?}: {e}"))?;
