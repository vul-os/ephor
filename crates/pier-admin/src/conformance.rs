//! Conformance self-check: run `pier_conformance::check` over the operator's current posture
//! and return the COORD-1..8 report, so an operator can see their own conformance — including
//! which clauses the harness can only mark [`pier_conformance::Outcome::Behavioral`] pending a
//! runtime test (CONTRACT §7, `pier-conformance`'s own honesty rule).
//!
...
//! descriptor's kind (the same table `pier_economics::kinds` documents), not an independent
...
//! delivery path) is exactly the gap `pier-conformance`'s `Behavioral` outcomes and W10's
...
impl Coordinator for Posture {
    ...
    fn settlement(&self) -> Settlement {
        ...
        // (DIRECTION §5) — `pier-billing`'s settlement seam is existing-assets-only throughout.
