...
The gateway is a member of the **pier** workspace (`crates/pier-gateway`) and builds its own
`pier-gateway` binary. It needs two shared libraries, tag-pinned from the kotva repo (see
[`Cargo.toml`](Cargo.toml)).

STALE SECTIONS BELOW, not yet re-verified after the move out of envoir or the project rename
to Pier: this file still calls those libraries `dmtap-core`/`dmtap-mail` (they are `kotva-core`
and `kotva-mail` now) and still links a `SEPARATION.md` that does not exist in this repo. The
...
# 2. Run it:
cargo run -p pier-gateway -- personal ./personal.toml
```
...
`pier-gateway personal <config.toml>` reads a flat `key = value` file; every key is optional with a
safe default (a fresh gateway resolves nobody and is **not** an open relay). Unknown keys and
malformed values are hard startup errors (fail-closed). `pier-gateway run` takes the same settings
from `GATEWAY_*` environment variables instead (handy for systemd/containers). See
[`examples/personal.toml`](examples/personal.toml) or `pier-gateway help` for the full key list:
...
Reference bridge implemented as a library (`pier_gateway`) plus the `pier-gateway` daemon,
...
`cargo test -p pier-gateway`.

## Repo split

HISTORY, not current state. This crate was extracted from the `vul-os/envoir` monorepo and briefly
stood alone before landing here as one crate of the pier workspace; the extraction runbook and its
precondition (a published monorepo tag for the shared crates) were recorded in a `SEPARATION.md`
that is **not** carried in this repo. What survived the move: the gateway depends on the shared
substrate libraries by git tag rather than by sibling path dep.
