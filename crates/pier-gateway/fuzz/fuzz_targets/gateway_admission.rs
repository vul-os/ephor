...
use pier_gateway::authz::{AdmissionError, IdentityRegistry};

// **Relocated from envoir** (`fuzz/fuzz_targets/gateway_admission.rs`, removed by envoir commit
// `620a68c` when the gateway moved to Pier) and retargeted at the real, current `gateway` crate:
// `envoir_gateway::authz::{AdmissionError, IdentityRegistry}` → `pier_gateway::authz::{AdmissionError,
