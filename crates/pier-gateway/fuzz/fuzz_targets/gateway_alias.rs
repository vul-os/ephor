...
use pier_gateway::forwarded_addr::{decode, encode};

// **Relocated from envoir, and RETARGETED** (`fuzz/fuzz_targets/gateway_alias.rs`, removed by
// envoir commit `620a68c` when the gateway moved to Pier).
//
...
// analogue anywhere in Pier's `gateway` crate (this crate's own key-derived alias,
// `pier_gateway::authz::key_derived_localpart`, is a one-way **content-address hash** of the key,
...
// **forwarded-address** SRS-style codec, `pier_gateway::forwarded_addr::{encode, decode}` (§7.10.2):
...
//     (`pier_gateway::forwarded_addr` module docs), mirroring the sibling wire-decoder fuzz
