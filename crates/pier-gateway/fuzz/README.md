# `pier-gateway` fuzz targets

A self-contained [cargo-fuzz](https://github.com/rust-fuzz/cargo-fuzz) project targeting the
`pier-gateway` crate's attacker-controlled parsing/verification boundaries. **Not** a member of the main
Pier workspace — `Cargo.toml` detaches it with its own `[workspace]` table, so libFuzzer's
...
(and this fuzz coverage) moved to Pier (Wave W2; see the doc comments in each
`fuzz_targets/*.rs` file for the exact provenance and any API adaptation).

## Targets

- **`gateway_admission`** — `pier_gateway::authz::IdentityRegistry::admit` (§7.9, §9): a connecting
...
- **`gateway_alias`** — `pier_gateway::forwarded_addr::{encode, decode}` (§7.10.2): the SRS-style
...
This project's Cargo manifest and target sources were written against the CURRENT `pier-gateway` public
