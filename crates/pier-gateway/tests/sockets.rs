...
use pier_gateway::attestation::{Attestation, AttestationKey, GwKeyResolver, StaticGwKeys};
use pier_gateway::inbound::{
    AbuseDecision, AntiAbuse, DeliveryOutcome, InboundGateway, KeyDirectory, MeshDelivery,
    RecipientKey,
};
use pier_gateway::outbound::{OutboundTransport, TransportResult};
use pier_gateway::{MxListener, SmtpTcpTransport};
...
fn self_signed() -> (CertificateDer<'static>, Arc<rustls::ServerConfig>) {
    ...
    let cfg =
        pier_gateway::server_config(vec![cert_der.clone()], key.into()).expect("server config");
    (cert_der, cfg)
}
