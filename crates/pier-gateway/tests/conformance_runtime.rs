//! Wave W10 — runtime conformance tests that DISCHARGE the `pier_conformance` Behavioral
//! findings for the `gateway` kind (COORD-1 and COORD-5, coordinator/CONTRACT.md §3/§4), plus a
//! COORD-6 runtime demonstration.
//!
//! `pier_conformance::check()` marks COORD-1 ("verify descriptor signature once kotva-core is
...
//! per-kind runtime tests (`crates/pier-conformance/src/lib.rs` module docs, STYLE §8). This
...
use pier_conformance::Coordinator;
use pier_gateway::attestation::{Attestation, AttestationKey};
use pier_gateway::coordinator::GatewayCoordinator;
...
use pier_gateway::spf::InMemorySpfResolver;

use pier_economics::visibility::VisibilityClass;
...
/// **Proves:** the gateway declares `terminating` at assurance `declared` (the one Pier kind
...
fn build_gw(
    ...
    gw.with_spf(Box::new(spf), SpfPolicy::Enforce)
        .with_dkim(Box::new(pier_gateway::dkim::StaticDkimKeys::new()), DkimPolicy::Annotate)
        .with_dmarc(
            Box::new(pier_gateway::dmarc::InMemoryDmarcResolver::new()),
            DmarcHandling::Annotate,
        )
}
...
/// narrowings exist in `pier_gateway::spf`/`dkim`/`dmarc`'s own module docs) — only that the delivery
