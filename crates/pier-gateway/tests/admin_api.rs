...
use pier_gateway::{AdminApi, AdminAuth, AdminServer, MultiDomainGateway, UsageMeter};
...
fn admin_api_over_tls_is_fail_closed_and_provisions_a_domain() {
    ...
    let ik_b64 = pier_gateway::b64::encode(&ik);
    let seal_b64 = pier_gateway::b64::encode(&[5u8; 32]);
    let (status, body) = request(
        addr,
        client.clone(),
        "POST",
