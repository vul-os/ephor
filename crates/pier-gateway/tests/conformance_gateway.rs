...
//! moved here, to Pier's `gateway` crate. Wave W2 (this file) retrieves those cases from envoir's
//! git history (`620a68c^`) and re-homes them here, driven against the REAL, current pier
//! `gateway` public API (not a copy or a stub) — `kotva_core` in place of `dmtap_core`, `pier_gateway::`
...
//!   "high-value recipient" policy concept to select between. Pier's `pier_gateway::attestation` /
//!   `pier_gateway::provenance` modules inherited that same single-tier design (one DNS-published
...
//! call shapes are unchanged (`pier_gateway::provenance::GatewayAttestation::{sign,verify}`,
//! `pier_gateway::provenance::chain_append`, `pier_gateway::outbound::OutboundGateway::{translate_and_sign,
//! send_authenticated}`, `pier_gateway::outbound_guard::OutboundSenderGuard`, `pier_gateway::authz::
//! AliasAllocator`, `pier_gateway::alias_map::GatewayAliasMap`, `pier_gateway::forwarded_addr::{encode,
...
//! pier's `pier_gateway::attestation::Attestation::verify` (`expected_domain, published_key,
...
use pier_gateway::alias_map::{AliasTarget, GatewayAliasError, GatewayAliasMap};
use pier_gateway::attestation::{AttestationError, AttestationKey};
use pier_gateway::authz::{AliasAllocator, AliasError};
use pier_gateway::forwarded_addr::{
    decode as fwd_decode, encode as fwd_encode, ForwardedAddrError,
};
use pier_gateway::outbound::{
    AlwaysRequireTls, GovernedSend, OutboundError, OutboundGateway, OutboundTransport,
    TransportResult,
};
use pier_gateway::outbound_guard::{OutboundSenderGuard, SenderVerdict};
use pier_gateway::provenance::{chain_append, GatewayAttestation, ProvenanceError};
...
/// Drives the real SRS-style `pier_gateway::forwarded_addr` codec: a dangling/ambiguous escape decodes
...
/// &[u8])`; pier's binds to the wrapped MOTE's content address instead, so `verify` here takes
...
// either), and `pier_gateway::attestation` / `pier_gateway::provenance` still model exactly the one flat
