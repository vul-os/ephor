...
use pier_gateway::attestation::{
    Attestation, AttestationError, AttestationKey, GwKeyResolver, StaticGwKeys,
};
use pier_gateway::dkim::{self, DkimError, DkimKey, StaticDkimKeys};
use pier_gateway::dmarc::InMemoryDmarcResolver;
use pier_gateway::inbound::{
...
use pier_gateway::mta_sts::{InMemoryPolicyFetcher, InMemoryTxtResolver, MtaStsTlsPolicy};
use pier_gateway::mx::{InMemoryMxResolver, MxHost};
...
fn inbound_dkim_annotate_delivers_regardless_of_verdict() {
    ...
    assert!(matches!(gw.verify_inbound_dkim(&tampered), pier_gateway::dkim::DkimVerdict::Fail(_)));
}
...
fn dmarc_enforce_rejects_a_message_with_multiple_from_headers() {
    ...
    assert_eq!(
        ...
        pier_gateway::dmarc::DmarcVerdict::Fail {
            disposition: pier_gateway::dmarc::DmarcDisposition::Reject
        },
...
fn malformed_binary_garbage_data_never_panics_with_spf_dkim_dmarc_all_enforcing() {
    ...
    assert_eq!(
        ...
        pier_gateway::dmarc::DmarcVerdict::PermError
    );
...
fn claim_test_gw(
    directory: pier_gateway::InMemoryDirectory,
...
fn address_claim_authz_refuses_a_sender_claiming_someone_elses_address_on_the_same_domain() {
    ...
    let directory = pier_gateway::InMemoryDirectory::new()
        .with_recipient(
            "alice@alice-domain.com",
...
fn address_claim_authz_allows_a_sender_claiming_their_own_address() {
    let alice_ik = kotva_core::identity::IdentityKey::generate();
    let directory = pier_gateway::InMemoryDirectory::new().with_recipient(
        "alice@alice-domain.com",
        RecipientKey { ik: alice_ik.public(), seal_pub: vec![0u8; 32] },
    );
    let transport = std::sync::Arc::new(RecordingTransport::new(
