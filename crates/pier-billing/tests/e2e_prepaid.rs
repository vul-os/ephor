...
use pier_billing::pricing::{recommended_tariff, CoordinatorPricingKind, HostingProfile};
use pier_billing::receipt::{BilledOperation, ReceiptLog};
use pier_billing::sim::{BillingEvent, BillingOutcome, SimEngine};
use pier_billing::{InMemoryLedger, PrepaidLedger, ResourceKind, SettlementRail, Subscription};
...
/// `pier_economics::Tariff`, verify it, decode it back, then run it through top-up → metered
...
fn e2e_full_prepaid_lifecycle_through_a_recommended_tariff() {
    ...
    let schedule = pier_billing::TariffSchedule::from_tariff(&signed_tariff).unwrap();

    // 2. The prepaid rail: a payer tops up, custody-free (Pier holds no funds — see
...
fn e2e_low_balance_and_exhausted_transitions() {
    ...
    let schedule = pier_billing::TariffSchedule {
...
/// `pier_economics::UsageReceipt` for an operation the prepaid ledger never actually debited
