//! Tariff evaluation — turning a signed [`pier_economics::Tariff`] + metered usage into an
//! amount owed (CONTRACT §6).
//!
//! `pier_economics::Tariff` carries an opaque, signed `schedule: Cbor` — the *mechanism*
...
//! concrete shape: it is built with [`pier_economics::Cbor::from_cv`] (so it rides the real
...
/// policy"). Signed into a [`pier_economics::Tariff`] via [`TariffSchedule::sign`]; parsed back
...
impl TariffSchedule {
    ...
    /// Encode as the [`pier_economics::Cbor`] payload a [`pier_economics::Tariff`] carries
    /// (built via `Cbor::from_cv`, per the module doc).
    pub fn to_cbor(&self) -> Cbor {
        Cbor::from_cv(&self.to_cv())
    }

    /// Sign this schedule into a [`pier_economics::Tariff`] with the coordinator's real
    /// substrate identity — thin sugar over `Tariff::sign(self.to_cbor(), ik)`; this crate does
    /// not reimplement signing (W3's `pier_economics::descriptor`).
    pub fn sign(&self, ik: &pier_economics::IdentityKey) -> pier_economics::Tariff {
        pier_economics::Tariff::sign(self.to_cbor(), ik)
    }

    /// Parse a schedule out of a signed [`pier_economics::Tariff`]'s `schedule` payload,
    /// *without* checking the signature — callers MUST call [`pier_economics::Tariff::verify`]
    /// first (fail-closed, SEC-1); this function only decodes bytes that are already trusted.
    pub fn from_tariff(tariff: &pier_economics::Tariff) -> Result<Self, BillingError> {
        Self::from_cbor(&tariff.schedule)
    }

    /// Decode a schedule from raw [`pier_economics::Cbor`] bytes.
