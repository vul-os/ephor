...
//! [`pier_economics::Tariff`]/[`pier_economics::UsageReceipt`] types from W3
//! (`pier-economics/src/descriptor.rs`). This crate does not reinvent signing, canonical CBOR,
//! or identity: it reuses `pier-economics`/`kotva-core` throughout and adds the *policy* layer
...
//!   `pier_economics::Cbor::from_cv`, and [`tariff::TariffSchedule::evaluate`] turning usage
...
//! - [`receipt`] — issuing signed [`pier_economics::UsageReceipt`]s for a [`tariff::Bill`]
...
//!   `pier-billing-patala` crate (a Stellar-backed prepaid top-up rail) — never a dependency of
//!   this crate.
...
//! denominated in a caller-supplied currency/asset string (DIRECTION §5), never a Pier-minted
...
mod one_directional_audit {
    ...
    //! `pier_economics` signing path (not a stand-in) — the residual is a property of the
