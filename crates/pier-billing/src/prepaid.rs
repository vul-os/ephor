...
//!
//! ## Pier holds no funds — "credit claims," not custody
//!
...
//! `crates/pier-billing-patala` for a real adapter that only calls this after independently
...
use pier_economics::{IdentityKey, UsageReceipt};
...
pub struct PrepaidLedger {
    ...
    /// not two fundings. Mirrors `pier-billing-patala`'s `credited_references` dedup.
...
mod tests {
    ...
    fn top_up_is_idempotent_on_funding_ref_no_double_credit() {
        ...
        // pier-billing-patala adapter already enforces on its own credited_references.
        let ledger = PrepaidLedger::new("USD", 100);
