...
//! [`pier_billing::settlement::SettlementRail`] (CONTRACT §6) backed by
...
//! it is **never** a dependency of `pier-billing` itself, and an operator who wants a different
...
//! `pier_billing::settlement` module docs.
//!
//! ## Prepaid, not per-charge network calls — and why `charge` stays synchronous
//!
//! [`pier_billing::settlement::SettlementRail::charge`] is a **synchronous** trait method.
...
//!
//! ## Non-custodial, by construction; Pier takes no cut
//!
...
//! it owns — mirroring `pier_billing::prepaid`'s own "credit claims, not custody" framing
...
//! For the OPTIONAL monthly-postpaid leg (`pier_billing::subscription::Subscription`), an
...
use pier_billing::settlement::{SettlementRail, SettlementReceipt};
...
pub enum PatalaSettlementError {
    ...
    /// same shape as [`pier_billing::prepaid::PrepaidError::Exhausted`].
...
/// A [`pier_billing::settlement::SettlementRail`] backed by ANY `patala_core::PaymentRail` (a
...
///
/// Like every `pier-billing` settlement adapter, this struct owns its own storage: a plain
...
/// `pier_billing::settlement::SettlementRail` never prescribes storage.
...
mod tests {
    ...
    #[tokio::test]
    async fn composes_with_pier_billing_subscription_exactly_like_any_other_rail() {
        // Proves this real, patala-backed adapter is a drop-in `SettlementRail` — no
        // patala-specific code needed in `pier_billing::subscription` itself.
        let rail = FakeRail::new("fake");
        let req = PayRequest {
            amount_minor: 2_000,
            currency: "USDC".into(),
        ...
        let subscription = pier_billing::Subscription::new(b"payer".to_vec(), 999, "USDC");
