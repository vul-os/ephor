...
//! - [`RelayCoordinator`] (this module) — the `pier_conformance::Coordinator` posture: kind,
//!   descriptor, and the CONTRACT §2/§4/§6 answers a relay operator gives. It composes a
//!   [`pier_economics::Descriptor`] signed by a real `kotva-core` `IdentityKey`; it does not
...
/// A `relay` coordinator's posture for `pier_conformance::check` (CONTRACT §2/§4/§6). Pairs a
...
impl RelayCoordinator {
    ...
    /// in the wrong kind/visibility gets exactly what `pier_conformance::check` is for: a
...
impl Coordinator for RelayCoordinator {
    ...
    fn settlement(&self) -> Settlement {
        ...
        // stablecoin/fiat rail; `pier-billing::SettlementRail` is where that composes in.
...
mod tests {
    ...
    fn backstop_relay_is_not_the_scarce_exception() {
        ...
        // if it really were a member of that class (pier-conformance checks membership, see
        // `pier_conformance::check`'s COORD-3 arm) — demonstrate the violation directly.
