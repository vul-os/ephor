...
//!   feature shape, BUILD-PLAN.md W5), so a pier relay and an envoir/dmtap-p2p mesh node
...
///
/// The path segment changed when the project was renamed to Pier — a peer that pins the
/// previous string will not match this one. Circuit Relay v2 itself is unaffected (that
/// protocol id is libp2p's, not ours); only what this server advertises via Identify changed.
const IDENTIFY_PROTOCOL: &str = "/pier/relay/id/1.0.0";
...
impl RelayServer {
    ...
    /// identity ([`crate::RelayCoordinator`]'s [`pier_economics::IdentityKey`]) is separate,
