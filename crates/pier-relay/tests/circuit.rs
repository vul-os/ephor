...
/// circuit). Distinct from [`pier_relay::server`]'s `RelayBehaviour`, which runs the relay **server**
    ...
    libp2p::SwarmBuilder::with_new_identity()
        ...
        .with_behaviour(|key, relay_client| {
            let identify = identify::Behaviour::new(identify::Config::new(
                "/pier/relay-test/id/1.0.0".to_string(),
                key.public(),
            ));
...
/// async-task-behind-a-channel shape [`pier_relay::server::RelayServer`] uses (and dmtap-p2p before
