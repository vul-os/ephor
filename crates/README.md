...
| `pier-cli` | `pier` — the flyctl-shaped CLI over the DEPOT control plane (`profiles/cloud.md` §5). Not a coordinator kind: a *client* of one. Every verb resolves to a `kotva_depot::Ability` value from the CLOSED §5.2 registry and a `kotva_depot::ResourceRef` from the §5.1 grammar, walked in both directions with counts by `tests/ability_mapping.rs`. | **no transport** — 39 tests. Not one subcommand reaches a coordinator: DEPOT-1's IK-authenticated Noise channel is not built. Real: the vocabulary, the fail-closed authorisation path, the `certs add` planner. Everything else prints what it WOULD send and exits non-zero. |
