...
//! a stake field from [`pier_economics::Descriptor`] so stake can never become a ranking signal;
...
//! stake (SEC-1, fail closed). This crate does not depend on `pier-billing` (kept intentionally
//! thin, matching the other posture scaffolds) — the seam a relying operator wires a real check
//! through is `pier_billing::StakeVerifier` (`crates/pier-billing/src/stake.rs`), whose
...
/// An `arbiter` coordinator's posture for `pier_conformance::check` (CONTRACT §2/§4/§5/§6). See
...
impl ArbiterCoordinator {
    ...
    /// `pier_conformance::check` finding. Prefer [`ArbiterCoordinator::signed`] for the common
