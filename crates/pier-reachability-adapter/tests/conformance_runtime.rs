...
use pier_economics::{Cbor, CoordinatorKind, Descriptor};
use pier_reachability_adapter::auth::authenticate_as_box;
use pier_reachability_adapter::ingress::{AdapterServer, IngressError};
use pier_reachability_adapter::tunnel::Registration;
use pier_reachability_adapter::{NameKind, ReachabilityAdapter};
...
/// deliberately separate, mirroring `pier-economics`'s own
...
fn reach1a_bare_vanity_is_declared_not_verified_even_though_genuinely_signed() {
    ...
    let r = pier_conformance::check(&a);
