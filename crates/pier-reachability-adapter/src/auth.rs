...
/// as `PIER-REACH-v0/...`, matching `pier-economics/src/descriptor.rs`'s own
...
///
/// WIRE-BREAKING RENAME: this tag's prefix changed when the project was renamed to Pier.
/// A signature produced under the previous prefix does NOT verify here, and a
/// box built against the previous prefix will fail this handshake — by design (that is what a
/// DS tag is for), but it is a wire change, not a cosmetic one. Both sides must be rebuilt.
pub const TUNNEL_AUTH_DS: &[u8] = b"PIER-REACH-v0/tunnel-auth\x00";
...
pub struct AuthenticatedRegistration {
    ...
    /// `kotva_core::identity::IdentityKey::public()` returns and `pier-economics/descriptor.rs`
