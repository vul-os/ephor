...
//! - [`MediaRelayCoordinator`] (this module) — the `pier_conformance::Coordinator` posture:
//!   kind, declared visibility, and the CONTRACT §2/§4/§6 answers a media-relay operator gives.
//!   It composes a [`pier_economics::Descriptor`] signed by a real `kotva-core` `IdentityKey`;
...
//!
//! Per the founder decision (pier `DECISIONS.md`, `[2026-07-23 sfu]`) and DIRECTION §7, "the
...
/// A `media-relay` coordinator's posture for `pier_conformance::check` (CONTRACT §2/§4/§6).
...
pub struct MediaRelayCoordinator {
    ...
    /// §6/COORD-7). Bandwidth metering via `pier-billing` is an optional seam — a media-relay
...
impl MediaRelayCoordinator {
    ...
    /// `pier_conformance::check` is for: a COORD-1/COORD-4 finding, not a silent acceptance.
...
impl Coordinator for MediaRelayCoordinator {
    ...
    fn metering(&self) -> Metering {
        ...
        // Wiring real receipts rides `pier-billing`'s `Meter`/`ReceiptLog` (optional; not
    ...
    fn settlement(&self) -> Settlement {
        ...
        // existing stablecoin/fiat rail; `pier-billing::SettlementRail` is where that composes
        // in.
...
mod tests {
    use super::*;
    use pier_conformance::{check, ScarceResource};
    ...
    fn backstop_media_relay_is_not_the_scarce_exception() {
        ...
        // only if it really were a member of that class (`pier_conformance::check`'s COORD-3
