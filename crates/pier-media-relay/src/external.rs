...
//!
//! Pier does **not** reimplement an SFU, SFrame (RFC 9605), or WebRTC (DIRECTION §3,
...
impl CoturnConfig {
    ...
    /// coturn's own config format, not a Pier invention (bind-don't-reinvent).
    pub fn to_conf_text(&self) -> String {
        let mut out = String::new();
        out.push_str("# generated by pier media-relay — coturn sidecar config (RFC 8656)\n");
        ...
        // fingerprint-free; a documented coturn hardening default, not a Pier invention.
...
/// (bind-don't-reinvent: Pier orchestrates LiveKit, it does not own LiveKit's schema).
...
mod tests {
    ...
    const DEFINITELY_ABSENT_BINARY: &str = "pier-media-relay-definitely-not-a-real-binary-xyz";
    ...
    fn coturn_config_generates_valid_conf_text() {
        let cfg = CoturnConfig::reference("pier.example", "s3cr3t");
        let text = cfg.to_conf_text();
        assert!(text.contains("listening-port=3478"));
        assert!(text.contains("realm=pier.example"));
    ...
    fn coturn_launch_command_references_the_config_file() {
        let cmd = CoturnConfig::launch_command("/etc/pier/turnserver.conf");
        assert_eq!(cmd[0], "turnserver");
        assert!(cmd.contains(&"/etc/pier/turnserver.conf".to_string()));
    }

    #[test]
    fn livekit_config_generates_valid_json_yaml_compatible_text() {
        let cfg = LiveKitConfig::reference("api-key", "api-secret", "turn.pier.example");
        let text = cfg
            .to_config_text()
        ...
        assert_eq!(parsed["turn"]["domain"], "turn.pier.example");
        assert_eq!(parsed["keys"]["api-key"], "api-secret");
    }

    #[test]
    fn livekit_launch_command_references_the_config_file() {
        let cmd = LiveKitConfig::launch_command("/etc/pier/livekit.yaml");
        assert_eq!(cmd[0], "livekit-server");
        assert!(cmd.contains(&"/etc/pier/livekit.yaml".to_string()));
    }
