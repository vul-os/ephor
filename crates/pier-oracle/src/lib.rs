...
//! kind** — the `pier_conformance::Coordinator` posture — not the `ATTEST` primitive itself,
...
//! from [`pier_economics::Descriptor`] by design, so stake can never become a ranking signal;
//! §6 requires it be verified **on the settlement/staking rail itself**, with an unverifiable claim
//! MUST-treated as no stake (SEC-1, fail closed). This crate does not depend on `pier-billing`
//! (kept thin, matching the other posture scaffolds) — the seam a relying operator wires a real
//! check through is `pier_billing::StakeVerifier` (`crates/pier-billing/src/stake.rs`), whose
...
/// An `oracle` coordinator's posture for `pier_conformance::check` (CONTRACT §2/§4/§5/§6). See
...
impl OracleCoordinator {
    ...
    /// `pier_conformance::check` finding. Prefer [`OracleCoordinator::signed`] for the common
