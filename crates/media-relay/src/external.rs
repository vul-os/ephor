...
//!
//! Ephor does **not** reimplement an SFU, SFrame (RFC 9605), or WebRTC (DIRECTION §3,
...
impl CoturnConfig {
    ...
    /// coturn's own config format, not an Ephor invention (bind-don't-reinvent).
    pub fn to_conf_text(&self) -> String {
        let mut out = String::new();
        out.push_str("# generated by ephor media-relay — coturn sidecar config (RFC 8656)\n");
        ...
        // fingerprint-free; a documented coturn hardening default, not an Ephor invention.
...
/// (bind-don't-reinvent: Ephor orchestrates LiveKit, it does not own LiveKit's schema).
...
mod tests {
    ...
    const DEFINITELY_ABSENT_BINARY: &str = "ephor-media-relay-definitely-not-a-real-binary-xyz";
    ...
    fn coturn_config_generates_valid_conf_text() {
        let cfg = CoturnConfig::reference("ephor.example", "s3cr3t");
        let text = cfg.to_conf_text();
        assert!(text.contains("listening-port=3478"));
        assert!(text.contains("realm=ephor.example"));
    ...
    fn coturn_launch_command_references_the_config_file() {
        let cmd = CoturnConfig::launch_command("/etc/ephor/turnserver.conf");
        assert_eq!(cmd[0], "turnserver");
        assert!(cmd.contains(&"/etc/ephor/turnserver.conf".to_string()));
    }
    ...
    fn livekit_config_generates_valid_json_yaml_compatible_text() {
        ...
        assert_eq!(parsed["turn"]["domain"], "turn.ephor.example");
        assert_eq!(parsed["keys"]["api-key"], "api-secret");
    }

    #[test]
    fn livekit_launch_command_references_the_config_file() {
        let cmd = LiveKitConfig::launch_command("/etc/ephor/livekit.yaml");
        assert_eq!(cmd[0], "livekit-server");
        assert!(cmd.contains(&"/etc/ephor/livekit.yaml".to_string()));
    }
