...
//!
//! This crate originally minted its own project-prefixed (`PIER-v0/...` in today's spelling)
//! object family and DS tags before the
//! spec had ratified a wire form (see the superseded `[2026-07-23 wire]` entry in
//! `COORDINATION.md`'s "Pier → Spec" section). The spec has since ratified §18.8a
...
//! papered over, at the `SUITE` constant below, in `COORDINATION.md` ("Pier → Spec",
...
/// doc's "Suite" section, `COORDINATION.md` ("Pier → Spec", `[2026-07-24 wire]`), and
...
pub enum DescriptorError {
    ...
    #[error(
        "unsupported/unrecognized signature suite {0:#04x} (fail closed, §1.1/§18.1.4) — this \
         implementation only verifies suite 0x01 (Ed25519); see COORDINATION.md \"Pier → Spec\""
    )]
    UnsupportedSuite(u8),
...
mod tests {
    ...
    //
    // Honest note on the three constants below: the pre-ratification family was prefixed with
    // this project's own name, and that prefix was re-spelled when the project was renamed to
    // Pier, so these are NOT the literal bytes the superseded code emitted. Nothing here depends
    // on them being those bytes: what these tests prove is that the OLD *layout* (key numbering,
    // absent `suite`) and a signature made under ANY tag other than the ratified
    // `DMTAP-COORD-v0/...` both fail closed under the current decoder/verifier. Changing the
    // prefix cannot weaken that — it only widens the set of tags proven not to verify. ---

    const OLD_DESCRIPTOR_DS: &[u8] = b"PIER-v0/coordinator-descriptor\x00";
    const OLD_TARIFF_DS: &[u8] = b"PIER-v0/tariff\x00";
    const OLD_USAGE_RECEIPT_DS: &[u8] = b"PIER-v0/usage-receipt\x00";
