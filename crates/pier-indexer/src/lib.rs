...
/// An `indexer` coordinator's posture for `pier_conformance::check` (CONTRACT §2/§4/§5/§6).
...
impl IndexerCoordinator {
    ...
    /// `pier_conformance::check` finding, not a silent acceptance. Prefer
...
impl Coordinator for IndexerCoordinator {
    ...
    fn metering(&self) -> Metering {
        ...
        // directly to the payer. Wiring real receipts rides `pier-billing`'s `ReceiptLog` —
...
mod tests {
    ...
    fn signed_indexer_descriptor_verifies_and_declares_terminating_by_default() {
        let (_coord, signed) = IndexerCoordinator::signed(
            &ik(1),
            QueryChannel::Terminating,
            Cbor::empty(),
            None,
    ...
    fn a_free_indexer_is_fully_conformant() {
        let (coord, _signed) = IndexerCoordinator::signed(
            &ik(3),
            QueryChannel::Terminating,
            Cbor::empty(),
            None,
